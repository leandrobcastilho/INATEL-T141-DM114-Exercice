package com.example.leandrocastilho.helowoordturbo.webservice;
...
import com.example.leandrocastilho.helowoordturbo.models.AccessToken;
import com.example.leandrocastilho.helowoordturbo.util.WSConstants;
import com.example.leandrocastilho.helowoordturbo.util.WSSalesUtil;
...
public class WebServiceSalesClient {

    private static final String GET_TOKEN = "/oauth/token";
    private static final String TAG = "WebServiceSalesClient";
    private static HttpURLConnection conn;
    private static AccessToken accessToken;

    private WebServiceSalesClient() {
    }
    ...
    private static WebServiceResponse authenticate(Context context) {
        String baseAddress = WSSalesUtil.getHostAddress(context);
        return authenticate(context, baseAddress);
    }

    private static WebServiceResponse authenticate(Context context, String baseAddress) {
        WebServiceResponse webServiceResponse = new WebServiceResponse();
        accessToken = null;
        try {
            ...
            conn.setRequestProperty("Accept", WSConstants.CONTENT_TYPE_JSON);
            conn.setRequestProperty("Content-Type", WSConstants.CONTENT_TYPE_URL_ENCODED);        //ClientId e secret (matilde:siecola em Base64)
            conn.setRequestProperty("Authorization", "Basic " + context.getString(R.string.authBasic));
            ...
            if (conn.getResponseCode() == 200) {
                ...
                editor.putString(context.getString(
                        R.string.pref_ws_sales_access_token),
                        accessToken.getAccess_token());
                editor.putLong(context.getString(
                        R.string.pref_ws_sales_access_token_expiration),
    ...
    private static void invalidateToken(Context context) {
        ...
        SharedPreferences sharedSettings = PreferenceManager.getDefaultSharedPreferences(context);
        SharedPreferences.Editor editor = sharedSettings.edit();
        editor.remove(context.getString(R.string.pref_ws_sales_access_token));
        editor.remove(context.getString(R.string.pref_ws_sales_access_token_expiration));
        editor.commit();
    }
    ...
    private static boolean isTokenValid(Context context) {
        ...
        if (sharedSettings.contains(context.getString(R.string.pref_ws_sales_access_token)) &&
                sharedSettings.contains(context.getString(R.string.pref_ws_sales_access_token_expiration))) {

            expirationTime = sharedSettings.getLong(context.getString(
                    R.string.pref_ws_sales_access_token_expiration), 0);

            if (Calendar.getInstance().getTime().getTime() < expirationTime) {
                accessToken = new AccessToken();
                accessToken.setAccess_token(sharedSettings.getString(
                        context.getString(R.string.pref_ws_sales_access_token), ""));
