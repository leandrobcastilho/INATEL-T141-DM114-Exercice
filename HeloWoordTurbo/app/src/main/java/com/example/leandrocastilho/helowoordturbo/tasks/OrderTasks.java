...
import com.example.leandrocastilho.helowoordturbo.webservice.WebServiceSalesClient;
...
public class OrderTasks {
    ...
    public OrderTasks(Context context, OrderEvents orderEvents) {
        ...
        baseAddress = WSSalesUtil.getHostAddress(context);
    }
    ...
    public void getOrders() {
        new AsyncTask<Void, Void, WebServiceResponse>() {
            ...
            protected WebServiceResponse doInBackground(Void... params) {
                return WebServiceSalesClient.get(context, baseAddress + GET_ORDERS);
            }
            ...
            protected void onPostExecute(WebServiceResponse webServiceResponse) {
                if (webServiceResponse.getResponseCode() == 200) {
                    Gson gson = new Gson();
                    try {
                        List<Order> orders = gson.fromJson(webServiceResponse.getResultMessage(), new TypeToken<List<Order>>() {
                        }.getType());
    ...
    public void getOrderById(long id) {
        new AsyncTask<Long, Void, WebServiceResponse>() {
            ...
            protected WebServiceResponse doInBackground(Long... id) {
                return WebServiceSalesClient.get(context, baseAddress + GET_ORDER_BY_ID + "/" + Long.toString(id[0]));
            }
