...
public class WebServiceMessageClient {
    ...
    private static final String TAG = "WebServiceMessageClient";
    private static HttpURLConnection conn;
    private static AccessToken accessToken;

    private WebServiceMessageClient() {
    }
    ...
    private static WebServiceResponse authenticate(Context context) {
        String baseAddress = WSMessageUtil.getHostAddress(context);
        ...
        try {
            ...
            if (conn.getResponseCode() == 200) {
                ...
                editor.putString(context.getString(
                        R.string.pref_ws_message_access_token),
                        accessToken.getAccess_token());
                editor.putLong(context.getString(
                        R.string.pref_ws_message_access_token_expiration),
    ...
    private static void invalidateToken(Context context) {
        ...
        SharedPreferences sharedSettings = PreferenceManager.getDefaultSharedPreferences(context);
        SharedPreferences.Editor editor = sharedSettings.edit();
        editor.remove(context.getString(R.string.pref_ws_message_access_token));
        editor.remove(context.getString(R.string.pref_ws_message_access_token_expiration));
        editor.commit();
    }
    ...
    private static boolean isTokenValid(Context context) {
        ...
        if (sharedSettings.contains(context.getString(R.string.pref_ws_message_access_token)) &&
                sharedSettings.contains(context.getString(R.string.pref_ws_message_access_token_expiration))) {

            expirationTime = sharedSettings.getLong(context.getString(
                    R.string.pref_ws_message_access_token_expiration), 0);

            if (Calendar.getInstance().getTime().getTime() < expirationTime) {
                accessToken = new AccessToken();
                accessToken.setAccess_token(sharedSettings.getString(
                        context.getString(R.string.pref_ws_message_access_token), ""));
